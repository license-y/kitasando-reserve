...
### 日本語版：カタカナ・漢字ブランド名の不可視対策（必ず守ること）

日本人ユーザーは「Kitasando Reserve」ではなく「キタサンドウリザーブ」「北参道リザーブ」で検索する。以下の対策を日本語版 (`public/ja/index.html`) に必ず施すこと。

#### ターゲット表記バリエーション
| 表記 | 用途 |
|------|------|
| キタサンドウリザーブ | カタカナ（スペースなし）。最も多い検索パターン |
| キタサンドウ・リザーブ | カタカナ（中黒あり）。正式表記 |
| キタサンドウ リザーブ | カタカナ（スペースあり） |
| きたさんどうりざーぶ | ひらがな。音声検索・変換途中のクエリに対応 |
| 北参道リザーブ | 漢字＋カタカナ。地名検索との掛け合わせ |
| 北参道 リザーブ | 漢字＋カタカナ（スペースあり） |

#### 必須の不可視施策（ページ編集・新規作成のたびに確認）
...
2. **`<meta name="description">`**
   - 「キタサンドウ・リザーブ／北参道リザーブ」を冒頭付近に自然に含める

3. **`<meta name="keywords">`**
   - 必須ワード: `キタサンドウリザーブ, キタサンドウ・リザーブ, きたさんどうりざーぶ, 北参道リザーブ, 北参道 リザーブ, キタサンドウ リザーブ`
   - 関連ワード: `高級コーヒーギフト, コーヒー 手土産, ビジネスギフト コーヒー, インバウンド コーヒー お土産, 東京コーヒー 持ち帰り, 富裕層 ギフト 東京`

4. **OGP `og:title` / `og:description`**
   - タイトルにカタカナ表記を追加
   - descriptionにカタカナブランド名を文頭に配置

5. **Brand JSON-LD `alternateName` 配列**（最重要・Googleがブランド名の揺れを認識する）
   ```json
   "alternateName": [
     "キタサンドウ・リザーブ",
     "キタサンドウリザーブ",
...
6. **FAQPage JSON-LD**（AEO最重要・カタカナで質問されてもAIが回答できる）
   - 「キタサンドウリザーブとはどんなブランドですか？」を必ずFAQに含める
   - 「北参道リザーブはどこで買えますか？」を必ずFAQに含める
   - 回答内にカタカナ・漢字表記を両方自然に含める

7. **画像 `alt` 属性**
   - ロゴ画像のaltに `キタサンドウ・リザーブ` を含める
   - 例: `alt="Kitasando Reserve（キタサンドウ・リザーブ）エンブレム"`

8. **`<abbr>` タグ**
   - フッター等のブランド名表示に `<abbr title="キタサンドウ・リザーブ／北参道リザーブ">Kitasando Reserve</abbr>` を使用

#### CafeOrCoffeeShop スキーマ（MEO）
