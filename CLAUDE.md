# プロジェクトの名前
Kitasando Reserve

## プロジェクト概要
- 非エンジニア、非デザイナーが、AI Agent (Claude Code)を使ってWebサイトを構築する
- 構築するWebサイトは「ワンページ完結」とする。１ページに必要な情報をまとめて提供する
- ブログなどは、別のサービス（ Substack や note.com など）を利用することを前提とする

## ファイル構成

- project-docs: Webページの内容や、その他作成に必要なドキュメントを格納
- public: Cloudflare Pages で公開設定をするディレクトリ。ここに成果物を入れる


# 作成のためのルール、ワークフロー

## 保存先

- ファイルを作成する際は、必ずUTF-8エンコーディングで保存し、日本語の文字化けが発生しないよう注意してください
- public ディレクトリに成果物を保存する
- 特にユーザーの指示がなければ、index.html で作成する
- 指示があれば、指示に合わせたファイルを publicの下に保存する
- 画像、CSS、Javascriptなどを生成した場合は、public/assets/ 以下の所定のフォルダに保存する

## コミット＆プッシュの確認（必ず守ること）

ファイルの変更・作成・削除を行った後は、必ずユーザーに「コミット＆プッシュしますか？」と確認すること。ユーザーの明示的な指示があるまで、自動でコミット・プッシュしない。

## あなたの仕事

- Webページを作成する: ユーザーの指示、意図を理解し、Webページを作成する。ユーザーがコンテンツも創造的に作って欲しいと判断したら、創造性を発揮して作る。ユーザーがコンテンツを事前に決定し、詳細に依頼してきた場合は、提示されたコンテンツに従って作成する
- Webページの編集: 編集箇所、依頼を理解して編集する
- Webページのチェック: 文法エラー、不要な記述、大きすぎる画像などがあれば、改善方法をアドバイスする

## コピーライティングのルール

- 日本語コピーは読点（、）をなるべく使わない。句点（。）や「——」で区切る
- 説明型ではなく示唆型・体験型のコピーを書く（「〜するブランドです」より「〜だ。」）

## 日本語版・英語版の両立（重要）

このサイトには日英2バージョンが存在する:
- 英語版: `public/index.html`
- 日本語版: `public/ja/index.html`

編集・修正を依頼された際、ユーザーが片方のバージョンしか言及していない場合は、作業完了後に必ず「日本語版（または英語版）にも同じ修正を反映しますか?」と確認すること。

## 画像の最適化について

- 画像の最適化が必要であれば、ユーザーに伝える
- 画像最適化については、以下の無料サービスを案内する:
    - **TinyPNG** (https://tinypng.com/): PNG・JPEG最適化の定番
    - **Squoosh** (https://squoosh.app/): Googleが提供する画像圧縮ツール
    - **Optimizilla** (https://imagecompressor.com/): 複数画像の一括処理対応


# 条件、技術スタック

## HTML、CSS、Javascript

### 使用するライブラリ

- Tailwind CSS v4.1 以上を使用し、コード量を減らしつつ、エラーが少ない簡潔なコードを書いてください。
- **重要**: Tailwind CSS v4は必ず以下の方法でCDNから読み込んでください（画像最適化の方が速度に寄与するので、ビルドプロセスは不要です）：
  ```html
  <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
  ```
  - **注意**: v3の`<link>`タグ形式は使用しないでください。v4では`<script>`タグを使用します
  - カスタムテーマを設定する場合は、`<style type="text/tailwindcss">`内で`@theme`ディレクティブを使用してください：
  ```html
  <style type="text/tailwindcss">
    @theme {
      --color-primary: #3b82f6;
      --font-sans: 'Noto Sans JP', sans-serif;
    }
  </style>
  ```
- Animate.css v4.1.1 以上を使用して、適度にアニメーションをつける（派手にならないように注意）
- スクロールアニメーションは、AOS v2.3 以上を使用するか、ユーザーの要求によっては、  Animate.css v4.1.1 + Intersection Observer API で実装してください
- アイコンや、ロゴは、Lucide v0.536.0 (https://lucide.dev/) を使ってください

### パフォーマンス最適化

- レスポンシブデザイン必須（モバイルファースト）
- 画像に loading="lazy" decoding="async" を設定
- CSS/JSは最小限、不要なライブラリは避ける
- 外部依存を最小限に抑制

### HTML構造

- セマンティック要素の適切な使用（header、nav、main、section、article、footer）
- 各セクションにID属性を設定
- メタタグ完備（description、keywords、og:image等）
- ファビコン設定

## デザイン要件

- Tailwind CSS の theme を使ってカラーパレットを設定（ユーザーの要求や、ユーザーの伝えたいコンテンツに最適なものを選び設定する）
- フォントは日本語に最適なものを選択、パフォーマンスを考慮する
- ホワイトスペースを効果的に使い、読みやすさを重視


### コンテンツ構造

- セクション構成を明確に（Hero、About、Services、Contact等）
- セクション構成の指示がなければ、自分で考える（しっかり伝わるように、ベストプラクティスに従って構成）
- A4 6ページ以上になるように、セクションは最低でも8個以上を考える（ユーザーの指示によっては、少なくても構わない）
- 各セクションにはID属性を付けて、ナビゲーションリンクを設置してください（ただし、セクション数が多すぎる場合は、適度に減らしてください）
- フォームには適切なvalidationを実装してください
- 適度に画像を挿入する（背景画像として使っても良い）
- コメントを適度に入れて、後から修正しやすくしてください
- ワンページで完結するように作成

### 画像について

- ユーザーからの指定がない場合、以下のコマンドでUnsplashから画像を検索して使用する
  ```bash
  node .claude/skills/unsplash-image-finder/unsplash-search.js "キーワード"
  ```
- APIキーは `.env.local` の `UNSPLASH_ACCESS_KEY` に設定済み（変更不要）
- 動作確認は `node .claude/skills/unsplash-image-finder/unsplash-search.js --check` で実行
- 取得した画像URLは `?w=800&q=80&fm=webp&fit=crop` の形式で最適化する
  - Hero画像: `w=1920`、セクション画像: `w=800`〜`w=1200`、サムネイル: `w=400`
- 画像リンクが切れていることが多いので確かめること
- **本番公開する画像はUnsplash等の外部URLに直リンクしない**。使用する画像が決まったら `curl` 等でダウンロードし、`public/assets/images/` 配下に保存してローカル参照に切り替える（外部サービス側の仕様変更・画像削除でリンク切れになるリスクを避けるため）
  - ダウンロード時は表示サイズに応じて `w=`（必要なら `h=` も）を指定する。`h` を省略すると元画像の縦横比のまま返り、意図せず数MB規模のファイルになることがある
  - レスポンスが実際にはWebP形式で返る場合（`fm=webp` 指定時など）、保存するファイルの拡張子も `.webp` にする（`file` コマンド等で実体を確認し、拡張子と一致させる）
  - 最終的なファイルサイズは1枚あたり200KB以下を目安に `q=`（quality）を調整する

### 画像ファイル名の命名規則（必ず守ること）

`public/assets/images/` 以下に画像を保存する際は、以下のルールに従うこと（Google画像検索対策）。

- **小文字・ハイフン区切り**にする（アンダースコア `_` や PascalCase は使わない）
  - 悪い例: `KitasandoLOGO_Gold.jpg` `concept_why-kitasando-reserve.jpg`
  - 良い例: `kitasando-reserve-logo-gold.jpg`
- **画像の内容が分かる説明的な名前**にする（`1.jpg` `IMG_1234.jpg` のような連番・汎用名は禁止）
  - 悪い例: `district/1.jpg`
  - 良い例: `district/dagayasando-street-tokyo.jpg`
- 可能な範囲でターゲットキーワード（`tokyo` `coffee` `gift` `kitasando` 等）を自然に含める
- 商品画像など順序管理が必要な場合は先頭に数字プレフィックスを付けてよいが、番号だけでなく内容を表す語も必ず含める（例: `1_reserve-blend.jpg`）
- ファイルをリネームする際は `git mv` を使い、履歴を保持する。あわせてHTML内の参照（`src`属性・JSON-LD・favicon・og:image等）を英日両ページで漏れなく更新すること

### ナビ部分について

ナビゲーションについて、特別な指示がない限り、以下の使用に従って作成すること。

#### 動作仕様
- `position: fixed` で画面上部に固定（`sticky` ではない）
- 初期状態：背景透明、リンク色は白系（Heroセクションの背景画像を考慮）
- スクロール50px以降：`backdrop-filter: blur(12px)` + 半透明背景（`rgba`で70%程度の透明度）
- リンク色はスクロール後に暗色系に切り替え（背景とのコントラスト確保）
- レスポンシブ：`lg:`ブレークポイントでデスクトップ/モバイル切り替え

#### アクセシビリティ（必須）
- `<nav>` に `aria-label` を設定
- モバイルメニューボタンに `aria-expanded`, `aria-controls`, `aria-label` を設定
- メニュー開閉時に `aria-expanded` を動的に更新
- メニュー開閉でアイコン切り替え（Lucideの `menu` ⇔ `x`）

#### パフォーマンス
- スクロールイベントは `requestAnimationFrame` でスロットリング（毎フレーム発火を防止）

#### デザイン（Claude判断）
- 具体的な色、トランジション速度、影の強さは全体デザインに合わせて判断


## SEO / MEO / AEO 対策（必ず守ること）

コンテンツ編集・追加の際は、以下の方針に従ってキーワードと構造を意識すること。

### キーワード優先順位（必ず守ること）

このサイトはインバウンド向け・ギフト向けを主軸とする。コンテンツ編集・追加の際はキーワードを以下の優先順位で意識すること。

1. **「東京」を最優先** — 「東京コーヒー」「東京のコーヒー」「東京ギフト」「東京土産」「東京 持ち帰り」を全セクションで自然に含める
2. **次に「kitasandoreserve」** — ブランド名を英字で。メタタグ・JSON-LD・本文に分散させる
3. **ローカルキーワードで補強** — 「北参道」「千駄ヶ谷」「ダガヤサンドウ」をMEO対策として本文に含める

### ターゲットキーワード

| キーワード | 用途 |
|-----------|------|
| 東京コーヒー | ブランドの核心軸。全セクションで自然に含める |
| ギフト / コーヒーギフト | ギフトブランドとしての定義。FAQ・コレクションで必須 |
| お土産 / 東京土産 | 日本語版で訪問者・帰省者向けクエリに対応 |
| インバウンド / 訪日旅行者 | 英語版で "souvenir" "bring home from Tokyo" と組み合わせる |
| 東京コーヒーを持ち帰る | AEO最重要ワード。FAQで直接回答できる構造にする |
| 高級ブランド / ラグジュアリー | 富裕層・ビジネスギフト層向け。過剰にならず品格で表現 |
| 富裕層 / プレミアム | 高価格帯ギフト需要。「静かな高品質」として訴求する |

### SEO（検索エンジン最適化）

- `<title>` と `<meta name="description">` に上記キーワードを自然に含める
- 日本語版では「東京」「ギフト」「お土産」「北参道」を優先
- 英語版では「Tokyo Coffee」「gift」「souvenir」「bring home」「inbound」を優先
- `<h1>` は各ページで1つ。最重要キーワードを含めること
- `<h2>` / `<h3>` にもキーワードを分散させる

### MEO（マップ・ローカル検索最適化）

- Schema.org の `CafeOrCoffeeShop` に住所・営業時間・電話番号を正確に記載する
- 「北参道」「千駄ヶ谷」「ダガヤサンドウ」「渋谷区」のローカルキーワードを本文に含める
- Google マップへのリンク（`https://www.google.com/maps/search/Green+Beans+Coffee+3-30-1+Sendagaya+Shibuya+Tokyo/`）をアクセスセクションに設置する
- 「近くのコーヒー」「北参道 カフェ」「千駄ヶ谷 コーヒー」などの near-me クエリに対応できる文言を含める

### AEO（アンサーエンジン最適化 / AI検索対策）

AEOとは、ChatGPT・Gemini・Perplexity などの AI が質問に回答する際に、このサイトのコンテンツが引用・参照されやすくするための最適化。

#### 必須対応
- FAQセクションには必ず `FAQPage` の JSON-LD スキーマを設置する
- FAQ の回答は「直接答えから始まる」構造にする（Yes/No → 理由 → 補足の順）
- 以下のクエリに対応するFAQを常に維持する:
  - 「東京コーヒーを持ち帰るには？」/ "How do I bring Tokyo coffee home?"
  - 「Kitasando Reserve はどこで買える？」/ "Where can I buy Kitasando Reserve?"
  - 「東京の高級コーヒーギフトは？」/ "What is a luxury coffee gift from Tokyo?"
  - 「ビジネスギフトに使えますか？」/ "Is it suitable as a business gift?"
  - 「外国人へのお土産に向いていますか？」/ "Good gift for international visitors?"
  - 「ラグジュアリーブランドですか？」/ "Is it a luxury brand?"

#### コンテンツの書き方
- 回答は簡潔に。AI が切り取りやすい 2〜3 文の構造を意識する
- 「はい/いいえ」で始まる回答はAEOに有利
- ブランド名（Kitasando Reserve）・場所名（Green Beans Coffee、北参道）・固有名詞を回答内に必ず含める
- FAQを追加・編集したら、必ず JSON-LD の `FAQPage` スキーマにも同じ内容を反映する

### 日本語版：カタカナ・漢字ブランド名の補助対策（不可視施策）

**メインのブランド名・SEOターゲットは `kitasandoreserve`（英字）である。**
カタカナ（キタサンドウリザーブ）・漢字（北参道リザーブ）は、日本語検索への補助対策として、**ユーザーの目に触れない箇所にのみ**記述する。可視コンテンツのメインはあくまで英字ブランド名で統一すること。

#### 補助対策の対象表記
| 表記 | 位置づけ |
|------|---------|
| キタサンドウ・リザーブ | 正式カタカナ表記。括弧書き程度の露出は可 |
| キタサンドウリザーブ | スペースなし。検索クエリとして最も入力される形 |
| きたさんどうりざーぶ | ひらがな。音声検索・変換中のクエリに対応 |
| 北参道リザーブ | 地名＋カタカナ。ローカル検索との掛け合わせ |
| キタサンドウ リザーブ / 北参道 リザーブ | スペースあり表記。表記揺れ対応 |

#### 不可視施策チェックリスト（日本語版編集のたびに確認）

1. **`<title>` タグ** — メインは英字。カタカナは括弧で補足する程度にとどめる
   - 例: `Kitasando Reserve（キタサンドウ・リザーブ）| 北参道コーヒーギフト`

2. **`<meta name="description">`** — 英字ブランド名が主体。カタカナは括弧書きで1回だけ自然に含める
   - 例: `Kitasando Reserve（キタサンドウ・リザーブ／北参道リザーブ）は…`

3. **`<meta name="keywords">`** — ユーザーには見えない。カタカナ・ひらがな・漢字の全表記を網羅する
   - 必須: `キタサンドウリザーブ, キタサンドウ・リザーブ, きたさんどうりざーぶ, 北参道リザーブ, 北参道 リザーブ, キタサンドウ リザーブ`
   - 関連: `高級コーヒーギフト, コーヒー 手土産, ビジネスギフト コーヒー, インバウンド コーヒー お土産, 東京コーヒー 持ち帰り, 富裕層 ギフト 東京`

4. **OGP `og:title` / `og:description`** — SNSシェア時に表示される。カタカナを括弧で1回添える程度

5. **Brand JSON-LD `alternateName`**（最重要・完全に不可視）
   - Googleがブランド名の表記揺れを同一エンティティとして認識するための宣言
   ```json
   "alternateName": [
     "キタサンドウ・リザーブ",
     "キタサンドウリザーブ",
     "きたさんどうりざーぶ",
     "北参道リザーブ",
     "北参道 リザーブ",
     "キタサンドウ リザーブ",
     "北参道コーヒーギフト"
   ]
   ```

6. **FAQPage JSON-LD**（AEO対応・完全に不可視）
   - カタカナ・漢字で質問されてもAI（ChatGPT/Gemini等）が回答できるよう、以下2問を必ずFAQスキーマに含める
   - 「キタサンドウリザーブとはどんなブランドですか？」
   - 「北参道リザーブはどこで買えますか？」
   - **HTMLのFAQセクションには表示しない**（JSON-LDのみ。ページ上ではあくまで英字ブランド名を主体とする）

7. **画像 `alt` 属性**（不可視）— ロゴ画像のaltにカタカナを含める
   - 例: `alt="Kitasando Reserve（キタサンドウ・リザーブ）エンブレム"`

8. **`<abbr>` タグ**（不可視） — フッター等のブランド名に `title` 属性でカタカナを付与
   - 例: `<abbr title="キタサンドウ・リザーブ／北参道リザーブ">Kitasando Reserve</abbr>`

#### CafeOrCoffeeShop スキーマ（MEO・完全に不可視）
- `hasMap`: Google マップURL を設定
- `sameAs`: Instagram・Google マップ を設定
- `keywords`: インバウンド・高級ギフト・焙煎体験 等のローカルワードを含める

### SEO/AEO 定期監査チェックリスト

ユーザーから「SEO監査して」「SEO/AEOを強化して」等の依頼があった場合、英語版・日本語版の両方で以下を確認すること。

**コンテンツ構造**
- まとめ（Summary）セクション: FAQセクションの直後・フッターの直前に設置する。冒頭に太字（`<strong>`）の直接回答リードを置き、各セクション（`#concept` `#story` `#philosophy` `#culture` `#collection` `#access` `#faq`）への内部リンクと、Green Beans Coffee（親ブランド）への外部リンクを自然文に含める
- 太字リード: Conceptセクション冒頭やまとめセクションなど、AIが直接引用しやすい箇所に `<strong>` で1〜2文の直接回答を入れる。サイト全体の細身のフォントを崩さないよう `font-medium` 程度に留め、太字を多用しない
- 重複タイトル: `<title>` は英日で重複させない。`<title>` と `<h1>` を完全一致させない
- 幽霊リンク: `href="#"` のような遷移先のないダミーリンクを作らない。すべて実在するセクションIDに紐付ける
- パンくず（BreadcrumbList）: 最低2階層（Home > ページ名）にする。1階層のみだとリッチリザルト対象になりにくい

**内部リンク**
- フッターに全セクションへのミニナビ（内部リンク）+ Green Beans Coffeeアンバサダーサイトへのリンク + 言語切替リンクを設置する
- ナビゲーションにも主要セクション（まとめを含む）への導線を用意する

**画像**
- alt属性には東京・北参道・Kitasando Reserve等のキーワードを自然に含める。特にPhilosophy/Cultureのような雰囲気写真は "Silence" "Latte art" のような汎用的な単語だけになりがちなので注意する
- ファイル名の命名規則（前述）を満たしているか確認する
- 1枚あたり200KB以下か、外部ドメインへの直リンクが残っていないか確認する（詳細は「画像について」を参照）

**構造化データ**
- Brand JSON-LDの `sameAs`・`alternateName` は英語版・日本語版で揃える（カタカナ表記の`alternateName`は日本語版限定の施策でよいが、`sameAs`のような客観的な情報は両方に入れる）
- LocalBusiness（CafeOrCoffeeShop）の電話番号（Green Beans Coffee: 03-6447-0681）は、schemaだけでなくAccessセクションなど視認できる本文にも表示し、NAP（店名・住所・電話）を一致させる
- Kitasando Reserve側の電話番号（03-6447-0695）を使う箇所（フッター等）は、どちらのブランドの番号か分かるようラベル・aria-labelを明記する

**OGP / SNSシェア**
- `twitter:title` / `twitter:description` を明示的に設定する（`og:` へのフォールバック任せにしない）
- `og:image:width` / `og:image:height` / `og:image:alt` を設定する
- `og:locale:alternate` で英日相互のロケールを宣言する

**その他**
- コンテンツを変更したら `public/sitemap.xml` の `lastmod` を更新する


## ブランドの切り分け（必ず守ること）

### Green Beans Coffee (GBC)
- 北参道（渋谷区千駄ヶ谷）にある実店舗のカフェ・ロースタリー
- 厳選30種類のコーヒー豆を扱う
- お客様がカフェとして訪れる場所（焙煎体験なども実施）
- 電話番号: 03-6447-0681
- アンバサダーサイト: https://greenbeanscoffeeambassador.com/
  - GBC に関する情報（営業時間・イベント・豆の詳細など）はこのサイトを参照すること

### Kitasando Reserve
- GBC が展開する東京のコーヒーギフト専用ブランド（レーベル）
- カフェではない。GBC 店頭で購入できるギフト用の豆のブランド
- 電話番号: 03-6447-0695

### 注意事項
- GBC と Kitasando Reserve を同一視しない
- "Kitasando Reserve" を訪問先・場所として表現しない（訪れるのは GBC）
- GBC はカフェとして独自アイデンティティを持つ（30種類の豆、ロースタリー）
- Kitasando Reserve は GBC の中のギフトプロダクトライン
